...
# 模型路径
# 本地运行可用绝对路径
# 若部署到 GitHub / Streamlit Cloud，请改成相对路径：
# MODEL_PATH = 'best_model_xgboost.pkl'
# =========================
MODEL_PATH = r"E:\新建文件夹\GDM相关\GDM\新新\4. 机器学习\best_model_xgboost.pkl"

# =========================
# 该 pkl 实际使用的 9 个变量
# 这里输入原始值即可
# 如果模型是 Pipeline(Scaler + XGBoost)，会自动标准化
# =========================
feature_names = [
    "TG",
    "HDL-C",
...
if st.button("🔍 预测 GDM 风险"):
    try:
        # 直接把原始值送入模型
        # 如果模型是 Pipeline，会自动先 scaler.transform 再预测
        ...
        # 如果是 Pipeline，则拿最后一步分类器做 SHAP
        # 输入到 SHAP 的数据使用 scaler 转换后的数据
